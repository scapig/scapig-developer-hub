...
class ApiDefinitionConnector @Inject()(appConfig: AppConfig, wsClient: WSClient) {
  ...
  val serviceUrl = appConfig.serviceUrl("scapig-definition")
  ...
  def fetchAllApis(): Future[Seq[APIDefinition]] = {
    wsClient.url(s"$serviceUrl/apis").get() map {
      ...
      case r: WSResponse => throw new RuntimeException(s"Invalid response from scapig-definition ${r.status} ${r.body}")
    }
  }
