...
import models.{APIIdentifier, Application, ApplicationNotFoundException, CreateApplicationRequest}
...
class ApplicationConnector @Inject()(appConfig: AppConfig, wsClient: WSClient) {
  ...
  def subscribeToApi(appId: String, apiIdentifier: APIIdentifier): Future[APIIdentifier] = {
    wsClient.url(s"$serviceUrl/application/$appId/subscription?context=${apiIdentifier.context}&version=${apiIdentifier.version}").post(Json.obj()) map {
      case response if response.status == Status.NO_CONTENT => apiIdentifier
      case r: WSResponse => throw new RuntimeException(s"Invalid response from application ${r.status} ${r.body}")
    }
  }

  def unsubscribeToApi(appId: String, apiIdentifier: APIIdentifier): Future[APIIdentifier] = {
