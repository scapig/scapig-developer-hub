...
class TapiPublisherConnector @Inject()(appConfig: AppConfig, wsClient: WSClient) {

    val serviceUrl = appConfig.serviceUrl("tapi-publisher")
    ...
    def fetchRaml(context: String, version: String): Future[Option[String]] = {
        wsClient.url(s"$serviceUrl/raml?context=$context&version=$version").get() map {
            ...
            case r: WSResponse => throw new RuntimeException(s"Invalid response from tapi-publisher ${r.status} ${r.body}")
        }
    }
}
